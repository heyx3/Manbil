...
public:
    ...
    //The different scalar properties of a pixel color.
    enum Channels
    {
        Red,
        Green,
        ...
        //Only applies to reading from a texture.

        GrayscaleAverage,
        GrayscaleMax,
        GrayscaleMin,

        //Only applies to reading from an array.
    ...
    static void ToArray(const sf::Image & inImg, Channels channel, Fake2DArray<UByte> & channelOut);
    static void ToArray(const sf::Image & inImg, Channels channel, Fake2DArray<float> & channelOut);
    ...
    static void ToImage(const Fake2DArray<UByte> & channelIn, Channels channel, sf::Image & outImg, UByte defaultColor = 0, UByte defaultAlpha = 0);
    static void ToImage(const Fake2DArray<float> & channelIn, Channels channel, sf::Image & outImg, float defaultColor = 0.0f, float defaultAlpha = 0.0f);
