...
MTextureCubemap::MTextureCubemap(const TextureSampleSettings3D& _settings,
...
void MTextureCubemap::Create(const TextureSampleSettings3D& texSettings,
...
void MTextureCubemap::ClearData(unsigned int newW, unsigned int newH)
{
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, ToGLenum(pixelSize), width, height,
                 0, GL_RGBA, GL_FLOAT, 0);
    if (usesMipmaps)
    {
...
std::string MTextureCubemap::SetDataFromFiles(std::string negXPath, std::string negYPath,
                                              std::string negZPath,
                                              std::string posXPath, std::string posYPath,
                                              std::string posZPath)
{
    return SetDataFromFiles(negXPath, negYPath, negZPath,
                            posXPath, posYPath, posZPath,
                            usesMipmaps);
}
std::string MTextureCubemap::SetDataFromFile(CubeTextureTypes face, std::string filePath,
                                             bool shouldUpdateMipmaps)
{
    ...
    if (img.getSize().x != width || img.getSize().y != height)
    {
        return "Cubemap size is " + std::to_string(width) + "x" + std::to_string(height) +
                  " but the image was " + std::to_string(img.getSize().x) + "x" +
                  std::to_string(img.getSize().y);
    }
  
    Bind();
    glTexImage2D(TextureTypeToGLEnum(face), 0, ToGLenum(pixelSize), width, height,
                                              ...
                                              bool useMipmapping)
{
    ...
    Bind();
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, negX.getPixelsPtr());
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, negY.getPixelsPtr());
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, negZ.getPixelsPtr());
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, posX.getPixelsPtr());
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, posY.getPixelsPtr());
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, ToGLenum(PS_8U), sNegX.x, sNegX.y,
                 0, GL_RGBA, GL_UNSIGNED_BYTE, posZ.getPixelsPtr());
    if (usesMipmaps)
    {
        glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
    }
...
bool MTextureCubemap::SetData ## colorType(const Array2D<pixelType>& negX, \
                                           const Array2D<pixelType>& negY, \
                                           const Array2D<pixelType>& negZ, \
                                           const Array2D<pixelType>& posX, \
                                           const Array2D<pixelType>& posY, \
                                           const Array2D<pixelType>& posZ, \
...
{ \
    ...
    Bind(); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, negX.GetArray()); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, negY.GetArray()); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, negZ.GetArray()); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, posX.GetArray()); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, posY.GetArray()); \
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, posZ.GetArray()); \
    if (usesMipmaps) \
    { \
        glGenerateMipmap(GL_TEXTURE_CUBE_MAP); \
    } \
...
bool MTextureCubemap::SetFace ## colorType(CubeTextureTypes face, const Array2D<pixelType>& pixelData, \
                                           bool shouldUpdateMips) \
{ \
    ...
    Bind(); \
    glTexImage2D(TextureTypeToGLEnum(face), 0, ToGLenum(pixelSize), width, height, \
                 0, glDataType, glPixelType, pixelData.GetArray()); \
...
bool MTextureCubemap::UpdateFace ## colorType(CubeTextureTypes face, const Array2D<pixelType>& newData, \
                                              bool updateMips, unsigned int offX, unsigned int offY) \
{ \
    if (!IsValidTexture() || !Is ## colorType ## Texture() || \
    ...
    \
    Bind(); \
    glTexSubImage2D(TextureTypeToGLEnum(face), 0, offX, offY, newData.GetWidth(), newData.GetHeight(), \
                    glDataType, glPixelType, newData.GetArray()); \
